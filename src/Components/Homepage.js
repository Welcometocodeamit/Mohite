...
export default function Homepage(props) {
  ...
  function getData(){
    ...
    let uName=name.charAt(0).toUpperCase()+name.slice(1,4)
    for(let i=0; i<arr.length; i++){
      if((arr[i].name.slice(0,4))==uName){
        let dataofarr=arr[i]
        data.push(dataofarr)
      }
    }
    props.getDataFunct(data)
  }
  return (
    <>
        <div id='homepage'>
            <div id='homediv'>
                <div id='righthomediv'>
                    <div><h1 style={{fontSize:'60px' ,fontWeight: '1000'}}>Find <span id='partner'>Partners</span> (CAs) <br/>available online</h1></div>
                    <div><p style={{fontSize:'15px'}}><b>CONNECT</b> with us where your services are listed and visible to a myriad of businesses seeking CA’s for compliance support</p></div>
                    <div id='inputdiv'>
                      <input type='text' placeholder='Search by name' id='inputname'/>
                      <Link to={'/service'}>
                ...
                <div id='lefthomediv'>
                    <div><img style={{marginTop:'30%'}} src={p1} alt=''/></div>
                    <div><img src={p2} alt=''/></div>
                    <div><img style={{marginTop:'15%'}} src={p3} alt=''/></div>
                </div>
            </div>
        </div>
        <Homepage2/>
